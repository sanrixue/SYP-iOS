...
#define BordColor [AppColor app_9color]
...
- (void)reloadData {
    ...
    _leftHeaderWidth = [_fDataSource widthForColumnHeader:self];
    _sectionHeaderHeight = [_fDataSource heightForSectionHeader:self];
    
    self.contentSize = CGSizeMake((_numberColumn)*_width+_leftHeaderWidth, (_numberSection)*_height+_sectionHeaderHeight);
...
- (void)loadseenItems {
    if (!_topLeftView.superview) {
        _topLeftView.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, _leftHeaderWidth, _sectionHeaderHeight);
    ...
    for (NSInteger section=0; section<_numberSection; section++) {
        ...
        for (NSInteger column=0; column<_numberColumn; column++) {
            ...
            CGRect rect = CGRectMake((column)*_width+_leftHeaderWidth, (section)*_height+_sectionHeaderHeight, _width, _height);
    ...
    for (NSInteger section=0; section<_numberSection; section++) {
        ...
        CGRect rect = CGRectMake(self.contentOffset.x+self.contentInset.left, (section)*_height+_sectionHeaderHeight, _leftHeaderWidth, _height);
        if ([self isOnScreenRect:rect]) {
    ...
    for (NSInteger column=0; column<_numberColumn; column++) {
        ...
        CGRect rect = CGRectMake((column)*_width+_leftHeaderWidth, self.contentOffset.y+self.contentInset.top, _width, _sectionHeaderHeight);
        if ([self isOnScreenRect:rect]) {
...
- (void)cellDoubleClickAction:(FormCell *)cell{
    if (_fDelegate) {
        if ([_fDelegate respondsToSelector:@selector(form:didDoubleClickCellAtIndexPath:)]) {
            [_fDelegate form:self didDoubleClickCellAtIndexPath:cell.indexPath];
        }
    }
}

...
- (void)setupToolbars {
    ...
    indexLabel.frame = CGRectMake(self.width - 80 - 20, self.height-35, 80, 15);
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if (error) {
//        [HudTool showMessage:SDPhotoBrowserSaveImageFailText];
        label.text = SDPhotoBrowserSaveImageFailText;
    }else {
//        [HudTool showMessage:SDPhotoBrowserSaveImageSuccessText];
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index {
    ...
    if (!_isLocal) {
        ...
        if (self.delegate && [_delegate respondsToSelector:@selector(photoBrowser:localImageForIndex:)]) {
            imageView.image = [self.delegate photoBrowser:self localImageForIndex:index];
        }
    }
